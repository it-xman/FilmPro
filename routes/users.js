...
router.post('/login', function (req, res, next) {
  ...
  user.findUserLogin(req.body.username, req.body.password, function (err, userSave) {
    if (userSave.length !== 0) {
      ...
      res.json({status: 0, data: {token: token_after, userInfo: userSave}, message: '登录成功'})
    } else {
...
//    这个接口同时用于密码的重置，需要用户登录
  if (req.body.repassword) {
    ...
    if (req.body.token) {
      ...
      if (!req.body.password) {
        res.json({status: 1, message: "用户的老密码错误"})
      }
      ...
      user.findUserPassword(req.body.username, req.body.userMail, req.body.userPhone, function (err, userFound) {
        if (userFound.length !== 0) {
          user.update({password: req.body.repassword}, function (err, userUpdate) {
            if (err) {
              res.json({status: 1, message: "更改错误", data: err})
            }
            res.json({status: 0, message: '更改成功', data: userUpdate})
...
//用户显示站内信，其中receive参数的意义当为1时是发送的内容，2是收到的内容
